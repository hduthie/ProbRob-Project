...
function p_img=projectPoint(Xr,Xl)
  ...
  iXr=inv(Xr);
  p_img=[-1;-1];
  pw=iXr(1:3,1:3)*Xl+iXr(1:3,4); #landmark in camera frame
  if (pw(3)<0)
     return;
  endif;
  p_cam=K*pw; #landmark in image frame
  iz=1./p_cam(3);
  p_cam*=iz;
  if (p_cam(1)<0 || 
      p_cam(1)>image_cols ||
...
function [H, b, chi_tot, num_inliers] = linearizeProjections(XR, XL, Zp, associations, num_poses, num_landmarks, kernel_threshold)
  global pose_dim;
  global landmark_dim;
  ...
  for measurement_num = 1:size(Zp, 2)
    pose_index = associations(1, measurement_num);
    landmark_index = associations(2, measurement_num);
    z = Zp(:, measurement_num);  % measured pixel [u; v]
    Xr = XR(:,:,pose_index);
    Xl = XL(:, landmark_index);
    ...
    % Robust kernel weighting (Huber-like)
    if chi > kernel_threshold
      w = sqrt(kernel_threshold / chi);
      chi = kernel_threshold;
    else
      w = 1.0;
      num_inliers += 1;
    ...
    % Construct weighted information matrix (Omega)
    Omega = eye(2) * w;

    % Compute indices in H/b
    pose_matrix_index = poseMatrixIndex(pose_index, num_poses, num_landmarks);
    landmark_matrix_index = landmarkMatrixIndex(landmark_index, num_poses, num_landmarks);

    % Weighted block updates
    H(pose_matrix_index:pose_matrix_index+pose_dim-1,
