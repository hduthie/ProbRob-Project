...
function [H, b, chi_tot, num_inliers] = linearizePoses(XR, Zr, associations, num_poses, num_landmarks, kernel_threshold)
  global pose_dim;
  global landmark_dim;
  ...
  for measurement_num = 1:size(Zr, 3)
    Omega = eye(12);
    % Omega(1:9, 1:9) *= 1e3; % Optional: emphasize rotation
    ...
    i_idx = int64(poseMatrixIndex(pose_i_index, num_poses, num_landmarks));
    j_idx = int64(poseMatrixIndex(pose_j_index, num_poses, num_landmarks));
    r = pose_dim;

    H(i_idx:i_idx+r-1, i_idx:i_idx+r-1) += Ji' * Omega * Ji;
    H(i_idx:i_idx+r-1, j_idx:j_idx+r-1) += Ji' * Omega * Jj;
    H(j_idx:j_idx+r-1, i_idx:i_idx+r-1) += Jj' * Omega * Ji;
    H(j_idx:j_idx+r-1, j_idx:j_idx+r-1) += Jj' * Omega * Jj;

    b(i_idx:i_idx+r-1) += Ji' * Omega * e;
    b(j_idx:j_idx+r-1) += Jj' * Omega * e;
  endfor
endfunction
